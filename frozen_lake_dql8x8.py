...
class FrozenLakeDQL():
    def __init__(self, learning_rate_a=0.001, discount_factor_g=0.9, network_sync_rate=10,
                 replay_memory_size=1000, mini_batch_size=32,modelname = "defalt"):
    ...
    def train(self, episodes, render=False, is_slippery=False):
        ...
        # Save policy
        torch.save(policy_dqn.state_dict(), f"{self.modelname}.pt")
    ...
    For example, the FrozenLake 8x8 map has 8x8=16 states numbered from 0 to 15. 
    ...
    def test(self, episodes, is_slippery=False):
        ...
        # Load learned policy
        policy_dqn = DQN(in_states=num_states, h1_nodes=num_states, out_actions=num_actions) 
        policy_dqn.load_state_dict(torch.load(f"{self.modelname}.pt"))
...
def main():
    ...
    parser.add_argument('--epoch', help='训练几轮')
    args = parser.parse_args()


    print(f"文件: {args.modelname}")
    print(f"epoch_number: {args.epoch}")
     # 使用参数创建 FrozenLakeDQL 对象
    frozen_lake = FrozenLakeDQL(learning_rate_a=args.learning_rate,
                              discount_factor_g=args.discount_factor,
                              network_sync_rate=args.sync_rate,
